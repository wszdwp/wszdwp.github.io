...
brief_data:
  summary_left:
    ...
    - label: "AWS Machine Learning Blog"
      text: "In this post, we walk through how we fine-tuned Qwen 2.5 7B Instruct for tool calling us…"
    - label: "AWS Machine Learning Blog"
      text: "In this post, we show how to implement a generative AI agentic assistant that uses both…"
    - label: "AWS Machine Learning Blog"
      text: "This blog post demonstrates how Windward helps enhance and accelerate alert investigatio…"
    - label: "Meta Engineering"
      text: "AI coding assistants are powerful but only as good as their understanding of your codeba…"
  summary_right:
    - label: "观点摘要"
      text: "读后看，这件事的价值在于：Amazon Bedrock AgentCore Gateway provides a centralized layer for managing how AI a…"
    - label: "观点摘要"
      text: "读后看，这件事的价值在于：A pilot program to support independent safety and alignment research and develop th…"
    - label: "观点摘要"
      text: "读后看，这件事的价值在于：Google AI Edge Gallery Terrible name, really great app: this is Google's official a…"
    - label: "观点摘要"
      text: "读后看，这件事的价值在于：Release: datasette-ports 0.2 No longer requires Datasette - running uvx datasette-p…"
    - label: "观点摘要"
      text: "读后看，这件事的价值在于：Release: scan-for-secrets 0.3 New -r/--redact option which shows the list of matche…"
  items:
    - title: "Build AI-powered employee onboarding agents with Amazon Quick"
      ...
      opinion: "原文重点：In this post, we walk through building a custom HR onboarding agent with Quick. 读后观点：这篇内容真正有价值的部分是 We show how to configure an agent that understands your organization’s processes, connects to your HR systems, and a…"
    - title: "Accelerate agentic tool calling with serverless model customization in Amazon SageMaker AI"
      ...
      opinion: "原文重点：In this post, we walk through how we fine-tuned Qwen 2.5 7B Instruct for tool calling using RLVR. 读后观点：这篇内容真正有价值的部分是 We cover dataset preparation across three distinct agent behaviors, reward function design with ti…"
    - title: "Building Intelligent Search with Amazon Bedrock and Amazon OpenSearch for hybrid RAG solutions"
      ...
      opinion: "原文重点：In this post, we show how to implement a generative AI agentic assistant that uses both semantic and text-based search using Amazon Bedrock, Amazon Bedrock AgentCore, Strands Agents and Amazon OpenSearch. 读后观点：这篇内容真…"
    - title: "From isolated alerts to contextual intelligence: Agentic maritime anomaly analysis with generative AI"
      ...
      opinion: "原文重点：This blog post demonstrates how Windward helps enhance and accelerate alert investigation processes by combining geospatial intelligence with generative AI, enabling analysts to focus on decision-making rather than…"
    - title: "How Meta Used AI to Map Tribal Knowledge in Large-Scale Data Pipelines"
      ...
      opinion: "原文重点：AI coding assistants are powerful but only as good as their understanding of your codebase. 读后观点：这篇内容真正有价值的部分是 When we pointed AI agents at one of Meta’s large-scale data processing pipelines – spanning four re… POS…"
    - title: "Connecting MCP servers to Amazon Bedrock AgentCore Gateway using Authorization Code flow"
      ...
      opinion: "原文重点：Amazon Bedrock AgentCore Gateway provides a centralized layer for managing how AI agents connect to tools and MCP servers across your organization. 读后观点：这篇内容真正有价值的部分是 In this post, we walk through how to configure A…"
